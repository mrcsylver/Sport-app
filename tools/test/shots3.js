...
(async()=>{
 ...
 /* shop: crests then banners */
 await pg.click('.tab[data-view="me"]'); await openSects(pg); await pg.waitForTimeout(500);
 await pg.evaluate(()=>document.querySelector('#shopCrests').scrollIntoView({block:'start'}));
 await pg.waitForTimeout(300);
 await pg.screenshot({path:path.join(OUT,'B1-crests.png')});
 await pg.evaluate(()=>document.querySelector('#shopBanners').scrollIntoView({block:'start'}));
 await pg.waitForTimeout(400);
 await pg.screenshot({path:path.join(OUT,'B2-banners.png')});
 const nCrest = (await pg.$$('#shopCrests .shop-c')).length;
 const nBan = (await pg.$$('#shopBanners .shop-b')).length;
 console.log('  crests:', nCrest, ' banners:', nBan);
 const overlap = await pg.evaluate(()=>GI_CRESTS.filter(c=>GI_AVATARS.includes(c)));
 console.log('  crest/avatar overlap:', overlap.length ? overlap : 'none');
 process.exit(0);
